...
namespace WWT.Providers
{
    public abstract partial class WWTWeb_XML2WTT : RequestProvider
    {
        ...
        protected class FileCabinet
        {
            protected List<FileEntry> FileList;
            private Dictionary<string, FileEntry> FileDirectory;
            public string Filename;
            ...
            public void AddFile(string filename, bool rename, string newName)
            {
                ...
                //  try
                {
                    ...
                    else if (filename.ToLower().StartsWith(TempDirectory.ToLower()))
                    {
            ...
            public void Extract(bool overwrite, string targetDirectory)
            {
                ...
                using (FileStream fs = File.OpenRead(Filename))
                {
                    ...
                    {
                        FileEntry fe = new FileEntry(child.Attributes["Name"].Value.ToString(),
                            Convert.ToInt32(child.Attributes["Size"].Value));
                        fe.Offset = offset;
                        offset += fe.Size;
                        FileList.Add(fe);
                    }
