...
namespace WWT.Maps
{
	internal sealed class HDPhotoDecoder : BitstreamDecoder
	{
		...
		{
			internal static readonly int[] cblkChromas = {0,4,8,16, 16,16,16, 0,0};
			//internal static readonly int[] cbChannels  = {2, 4};
			...
			{ 
				new int[] {0,5,1,6, 10,12,8,14, 2,4,3,7, 9,13,11,15}, //AC 444
				new int[] {0,5,1,6, 10,12,8,14, 2,4,3,7, 9,13,11,15}, //AC 420
				new int[] {0,128,64,208, 32,240,48,224, 16,192,80,144, 112,176,96,160 } //DC 444
			};
	    ...
	    private int[] iBlockDC = new int [16];	// assuming only one channel
		...
		/// </summary>
		/// <param name="data">Bytestream of HD Photo image.</param>
		...
		internal short[,] Decode(byte[] data)
		{
			const string devInfo = "Decode";
			...
			if (data != null)
			{
				...
				{
                    //lElevationAdjustmentServicePlugin.LogException(
                    //    new ExceptionInfo(e),
                    //    devInfo,
                    //    true);
					return null;
		...
		private void OutputMBRow()
		{
			...
			int iFirstRow = 0;
			int iFirstColumn = 0;
			int cHeight = Math.Min(cROIBottomY + 1 - (cRow - 1) * 16, 16);
			int iShift = 3;
			int iBias = 3;

			OutputNChannel(iFirstRow, iFirstColumn, (int)cWidth, cHeight, iShift, iBias);

        }

		// write one MB row of Y_ONLY/CF_ALPHA/YUV_444/N_CHANNEL to output buffer
		private void OutputNChannel(int iFirstRow, int iFirstColumn, int cWidth, int cHeight, int iShift, int iBias)
		{
			int rowOffset = (cRow - 1) << 4;

			for (int iRow = iFirstRow; iRow < cHeight; iRow++)
			{
				for (int iColumn = iFirstColumn; iColumn < cWidth; iColumn++)
				{
		...
		private void AdvanceOneMBRow()
		{
			...
			for (int i = 0; i < cmbWidth; i++)
			{
				pPredInfo = PredInfo[i];
		...
		{
		    bool bottom = cRow == cmbHeight;
		    bool bottomORright = (bottom || cColumn == cmbWidth);
		...
		*************************************************************************/
		...
		private void Transform()
		{
			...
			bool bottomORright = (bottom || right);
		    ...
		    if (OVERLAP.OL_NONE != olOverlap)
		    {
				if (leftORright)
				{
					int j = left ? 0 + 10 : -64 + 14;
					if (!top)
					{
						int k = j + p0MBbuffer.Pointer + 16;
		...
		*************************************************************************/
		private void PredACDec()
		{
			...
			switch (2-iOrientation)
			{
				...
				{
					// predict from top
					for (int j = 0; j < Constant.blkIdx.Length; ++j)
					{
						int k = p1MBbuffer.Pointer + (Constant.blkIdx[j] << 4);
		...
		*************************************************************************/
		...
		/*  8  9 12 13 */
		/* 10 11 14 15 */
		[System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Maintainability", "CA1500:VariableNamesShouldNotMatchFieldNames", MessageId = "iCBP", Justification = "To simplify comparizons with the original code")]
		...
		*************************************************************************/
		...
		{
			int iCBPCY = 0;
			int iNumCBP = m_pCodingContext.m_pAdaptHuffCBPCY1.GetHuffShort(bitIO);
			...
			for (int iBlock = 0; iBlock < 4; iBlock++)
			{
				...
				{
			        int iNumBlockCBP =  m_pCodingContext.m_pAdaptHuffCBPCY.GetHuffShort(bitIO);
			        uint val = (uint)iNumBlockCBP + 1/*, iCode1*/;
					...
					uint iCode1 = Constant.gOff0[val];
		...
		private void PredDCACDec()
		{
			...
			if(iADPredMode == 4)
			{// predict AD from top
			    int[] pRef = PredInfoPrevRow[cColumn].iAD;
				pOrg[4] += pRef[3];
				pOrg[8] += pRef[4];
				pOrg[12] += pRef[5];
			}
				...
				{// predict AD from left
					int[] pRef = PredInfo[cColumn - 1].iAD;
					pOrg[1] += pRef[0];
					pOrg[2] += pRef[1];
					pOrg[3] += pRef[2];
				}
		...
		{
			int StrH = (int)Math.Abs(iBlockDC[1]) + (int)Math.Abs(iBlockDC[2]) + (int)Math.Abs(iBlockDC[3]);
			int StrV = (int)Math.Abs(iBlockDC[4]) + (int)Math.Abs(iBlockDC[8]) + (int)Math.Abs(iBlockDC[12]);
		...
		*************************************************************************/
		...
		{
			CCodingContext.CAdaptiveScan[] pScan = m_pCodingContext.m_aScanLowpass;
		    int iModelBits = m_pCodingContext.m_aModelLP.m_iFlcBits[0];
			int[] aRLCoeffs = new int[32];
			int iNumNonzero = 0, iIndex = 0;
		    int[] aLaplacianMean = new int[2];
		    int iCBP = 0;
		    ...
		    iCBP |= bitIO.GetBit16(1);

	        int[] pCoeffs = iBlockDC;

	        if ((iCBP & 1) != 0) 
			{
	            iNumNonzero = DecodeBlock(aRLCoeffs);
			    ...
			    for (int k = 0; k < iNumNonzero; k++)
				{
					...
					{
			            CCodingContext.CAdaptiveScan cTemp = pScan[iIndex];
			...
			if (iModelBits != 0)
			{
				int iMask = (1 << iModelBits) - 1;
				for (int k = 1; k < 16; k++)
				{
					if (pCoeffs[k] != 0)
					{
						uint r1 = RotL((uint)pCoeffs[k], iModelBits);
						pCoeffs[k] = ((int)r1 ^ (int)bitIO.GetBit16((uint)iModelBits)) - ((int)r1 & iMask);
					}
					else
					{
			...
			HDPhotoDecoder.UpdateModelMB(aLaplacianMean, m_pCodingContext.m_aModelLP);
		...
		*************************************************************************/
		...
		{
			int[] aLaplacianMean = new int[2];
			int iModelBits = m_pCodingContext.m_aModelDC.m_iFlcBits[0];
			...
			if (cfColorFormat == COLORFORMAT.Y_ONLY || cfColorFormat == COLORFORMAT.CMYK || cfColorFormat == COLORFORMAT.N_CHANNEL)
			{
			    ...
			    iModelBits = m_pCodingContext.m_aModelDC.m_iFlcBits[1];
			}

			HDPhotoDecoder.UpdateModelMB(cfColorFormat, (int)cNumChannels, aLaplacianMean, m_pCodingContext.m_aModelDC);
		}
		...
		*************************************************************************/
		...
		private void AdaptHighpassDec()
		{
			...
			for (int kk = 0; kk < Constant.CONTEXTX; kk++)
			{
		...
		*************************************************************************/
		...
		{
			for (int kk = 0; kk < Constant.CONTEXTX + Constant.CTDC; kk++)
			{
...
//#if DEBUG
//        /// <summary>
//        /// Calll this function to output most variables used in decoding process to a file in Isolated Storage
//        /// </summary>
...
//                System.IO.IsolatedStorage.IsolatedStorageFileStream file = store.CreateFile("AltFromDEM Silverlight.log");

//                int i;

//                WriteLine(file, "=================== DEBUG INFO =========================");

//                for (int j = 0; j < 17; j++)
//                {
//                    WriteLine(file, "a0MBbuffer[" + j + "] a1MBbuffer[" + j + "]");
//                    for (i = 0; i < 256; i++)
//                        WriteLine(file, a0MBbuffer[(uint)(i+j*256-a0MBbuffer.Pointer)] + " " + a1MBbuffer[(uint)(i+j*256-a1MBbuffer.Pointer)]);
//                }

//                WriteLine(file, "cBitLeft: " + bitIO.cBitsUsed);

//                WriteLine(file, "iBlockDC");
//                for (i = 0; i < 16; i++)
//                    WriteLine(file, iBlockDC[i].ToString(CultureInfo.InvariantCulture));

//                WriteLine(file, "iCBP: " + iCBP);
//                WriteLine(file, "iDiffCBP: " + iDiffCBP);
//                WriteLine(file, "iOrientation: " + iOrientation);

//                WriteLine(file, "m_iCount0: " + m_pCodingContext.m_aCBPModel.m_iCount0[0] + " " + m_pCodingContext.m_aCBPModel.m_iCount0[1]);
//                WriteLine(file, "m_iCount1: " + m_pCodingContext.m_aCBPModel.m_iCount1[0] + " " + m_pCodingContext.m_aCBPModel.m_iCount1[1]);
//                WriteLine(file, "m_iState: " + m_pCodingContext.m_aCBPModel.m_iState[0] + " " + m_pCodingContext.m_aCBPModel.m_iState[1]);

//                WriteLine(file, "m_aModelAC.m_iFlcBits: " + m_pCodingContext.m_aModelAC.m_iFlcBits[0] + " " + m_pCodingContext.m_aModelAC.m_iFlcBits[1]);
//                WriteLine(file, "m_aModelAC.m_iFlcState: " + m_pCodingContext.m_aModelAC.m_iFlcState[0] + " " + m_pCodingContext.m_aModelAC.m_iFlcState[1]);
//                WriteLine(file, "m_aModelAC.mband: " + (int)m_pCodingContext.m_aModelAC.m_band);

//                WriteLine(file, "m_aModelDC.m_iFlcBits: " + m_pCodingContext.m_aModelDC.m_iFlcBits[0] + " " + m_pCodingContext.m_aModelDC.m_iFlcBits[1]);
//                WriteLine(file, "m_aModelDC.m_iFlcState: " + m_pCodingContext.m_aModelDC.m_iFlcState[0] + " " + m_pCodingContext.m_aModelDC.m_iFlcState[1]);
//                WriteLine(file, "m_aModelDC.mband: " + (int)m_pCodingContext.m_aModelDC.m_band);

//                WriteLine(file, "m_aModelLP.m_iFlcBits: " + m_pCodingContext.m_aModelLP.m_iFlcBits[0] + " " + m_pCodingContext.m_aModelLP.m_iFlcBits[1]);
//                WriteLine(file, "m_aModelLP.m_iFlcState: " + m_pCodingContext.m_aModelLP.m_iFlcState[0] + " " + m_pCodingContext.m_aModelLP.m_iFlcState[1]);
//                WriteLine(file, "m_aModelLP.mband: " + (int)m_pCodingContext.m_aModelLP.m_band);

//                WriteLine(file, "i m_aScanHoriz[i].uScan m_aScanHoriz[i].uTotal m_aScanVert[i].uScan m_aScanVert[i].uTotal m_aScanLowpass[i].uScan m_aScanLowpass[i].uTotal");
//                for (i = 0; i < 16; i++)
//                    WriteLine(file, i + " " +
//                            m_pCodingContext.m_aScanHoriz[i].uScan + " " + m_pCodingContext.m_aScanHoriz[i].uTotal+ " " +
//                            m_pCodingContext.m_aScanVert[i].uScan + " " + m_pCodingContext.m_aScanVert[i].uTotal+ " " +
//                            m_pCodingContext.m_aScanLowpass[i].uScan + " " + m_pCodingContext.m_aScanLowpass[i].uTotal);

//                WriteLine(file, "m_iCBPCountMax m_iCBPCountZero m_iTrimFlexBits");
//                WriteLine(file, m_pCodingContext.m_iCBPCountMax + " " + m_pCodingContext.m_iCBPCountZero + " " + m_pCodingContext.m_iTrimFlexBits);

//                for (i = 0; i < 21; i++)
//                    m_pCodingContext.m_pAHexpt[i].PrintCAdaptiveHuffman(file,"m_pAHexpt", i);
//                m_pCodingContext.m_pAdaptHuffCBPCY.PrintCAdaptiveHuffman(file, "m_pAdaptHuffCBPCY", 0);
//                m_pCodingContext.m_pAdaptHuffCBPCY1.PrintCAdaptiveHuffman(file, "m_pAdaptHuffCBPCY1", 0);

//                WriteLine(file, "pQuantizerDC: iIndex iQP iOffset iMan iExp");
//                WriteLine(file, pQuantizerDC.iExp + " " +  pQuantizerDC.iQP + " " +  pQuantizerDC.iOffset + " " +  pQuantizerDC.iMan + " " +  pQuantizerDC.iExp);

//                WriteLine(file, "pQuantizerLP: iIndex iQP iOffset iMan iExp");
//                WriteLine(file, pQuantizerLP.iExp + " " +  pQuantizerLP.iQP + " " +  pQuantizerLP.iOffset + " " +  pQuantizerLP.iMan + " " +  pQuantizerLP.iExp);

//                WriteLine(file, "pQuantizerHP: iIndex iQP iOffset iMan iExp");
//                WriteLine(file, pQuantizerHP.iExp + " " +  pQuantizerHP.iQP + " " +  pQuantizerHP.iOffset + " " +  pQuantizerHP.iMan + " " +  pQuantizerHP.iExp);

//                for (i = 0; i < 1; i++)
//                {
//                    WriteLine(file, "PredInfo " + i + ": iCBP iDC iQPIndex");
//                    WriteLine(file, PredInfo[i].iCBP + " " +  PredInfo[i].iDC + " " +  PredInfo[i].iQPIndex);
//                    WriteLine(file, PredInfo[i].iAD[0] + " " +  PredInfo[i].iAD[1] + " " + 
//                                                       PredInfo[i].iAD[2] + " " +  PredInfo[i].iAD[3] + " " + 
//                                                       PredInfo[i].iAD[4] + " " +  PredInfo[i].iAD[5]);
//                }
...
//        internal static void WriteLine(System.IO.IsolatedStorage.IsolatedStorageFileStream file, string str)
//        {
//            byte[] strByte = new byte[4096];

//            str += "\r\n";
//            for (int i = 0; i < str.Length; i++)
//                strByte[i] = (byte)str[i];

//            file.Write(strByte, 0, str.Length);
//        }
//#endif
//        #endregion
	}
